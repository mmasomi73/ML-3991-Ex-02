    ...
    def findAnomalies(self, saveChart=False, saveEvaluation=False):
        ...
        if saveChart:
            for i in range(len(predicted_outlier)):
                ...
                plt.title('OneClassSVM On File [{}]'.format(i + 1), **csfont)

                predicted_outlier[i].plot(figsize=(12, 6), label='predictions', marker='o', markersize=5)
                true_outlier[i].plot(marker='o', markersize=2)

                # data = list_of_df[i]
                # plt.scatter(x=data[data['ocsvm_anomaly'] == data['anomaly']].index,
                #             y=data[data['ocsvm_anomaly'] == data['anomaly']]['anomaly'], label='True Prediction'
                #             , c='g', zorder=4)
                # plt.scatter(x=data[data['ocsvm_anomaly'] != data['anomaly']].index,
                #             y=data[data['ocsvm_anomaly'] != data['anomaly']]['anomaly'], label='False Prediction'
                #             , c='r', zorder=5)
                plt.legend(loc='upper right')
                plt.savefig(self.path_to_plt + 'anom/ocsvm-pre-{}.png'.format(i + 1), format='png')
        ...
        if saveChart:
            ...
            for df in list_of_df:
                ...
                sns = sb.scatterplot(data=df, x='X', y='Y', hue='ocsvm_anomaly', palette='bright')
                sns.set_title('The Anomaly Detected By OneClassSVM, File {}'.format(ts))
        ...
        if saveEvaluation:
            ...
            data = {'far': round(FP / (FP + TN) * 100, 2),
                    ...
                    'fp': FP,
                    'fn': FN}
            output = OutputWriter(self.path_to_plt, 'OneClassSVM', data)
            output.write()

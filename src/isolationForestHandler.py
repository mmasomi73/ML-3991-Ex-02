    ...
    def findAnomalies(self, saveChart=False):
        ...
        if saveChart:
            for i in range(len(predicted_outlier)):
                ...
                plt.savefig(self.path_to_plt + 'anom/iso_for-pre-{}.png'.format(i + 1), format='png')
        ...
        if saveChart:
            ...
            for df in list_of_df:
                ...
                sns = sb.scatterplot(data=df, x='X', y='Y', hue='forest_anomaly', palette='bright')
                sns.set_title('The Anomaly Detected By Isolation Forest, File {}'.format(ts))
